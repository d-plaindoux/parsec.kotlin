...
class T02_ElementParser {
    ...
    fun shouldAnyParserReturnsAccept() {
        ...
        Assert.assertEquals(parser(Readers.new("a")).fold({ it.value == 'a' && it.consumed }, { false }), true)
    }
    ...
    fun shouldEOSParserReturnsAccept() {
        ...
        Assert.assertEquals(parser(Readers.new("")).fold({ true }, { false }), true)
    }
    ...
    fun shouldEOSParserReturnsReject() {
        ...
        Assert.assertEquals(parser(Readers.new("a")).fold({ false }, { true }), true)
    }
    ...
    fun shouldChoiceWithBacktrackParserReturnsAccept() {
        ...
        Assert.assertEquals(parser(Readers.new("a")).fold({ true }, { false }), true)
    }
    ...
    fun shouldNotSatisfyOrAnyParserReturnsAccept() {
        ...
        Assert.assertEquals(parser(Readers.new("b")).fold({ it.value == 'b' && it.consumed }, { false }), true)
    }
