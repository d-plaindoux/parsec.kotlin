...
class T03_OccurrenceParser {
    ...
    fun shouldOptionalParserWithEmptyStringReturnsAccept() {
        ...
        assertEquals(parser(Reader.new("")).fold({ !it.value.first.isPresent }, { false }), true)
    }
    ...
    fun shouldOptionalParserWithNonEmptyStringReturnsAccept() {
        ...
        assertEquals(parser(Reader.new("a")).fold({ it.value.first.get() == 'a' }, { false }), true)
    }
    ...
    fun shouldRepeatableParserWithNonEmptyStringReturnsAccept() {
        ...
        assertEquals(parser(Reader.new("ab")).fold({ it.value.first == listOf('a','b') }, { true }), true)
    }
}